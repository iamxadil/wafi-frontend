// src/components/query/useCategoryDynamicFilters.jsx
import { useQuery } from "@tanstack/react-query";
import axios from "axios";

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";

export const useCategoryDynamicFilters = ({ category = "", brand = "" } = {}) => {
  return useQuery({
    queryKey: ["categoryDynamicFilters", { category, brand }],
    queryFn: async () => {
      const params = { category };
      if (brand) params.brand = brand;

      const res = await axios.get(`${API_URL}/api/products/dynamic-filters`, {
        params,
      });

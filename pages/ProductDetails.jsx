...
const ProductDetails = () => {
  ...
  const handleSubmitReview = async () => {
    ...
    if (alreadyReviewed) return toast.info(t("You already reviewed this product", "لقد قمت بمراجعة هذا المنتج مسبقًا"));
    if (newRating < 1 || newRating > 5)   return toast.warning(t("Select a rating between 1 and 5", "اختر تقييماً بين 1 و 5"));
    if (!newComment.trim()) return toast.warning(t("Comment cannot be empty", "لا يمكن ترك التعليق فارغاً"));
  ...
  if (!window.confirm(t("Are you sure you want to delete this review?", "هل أنت متأكد أنك تريد حذف هذه المراجعة؟"))) return;
          ...
          <section className='pr-details-container'>
            <div className="dt-name">
              ...
              <p>
                ...
                {selectedProduct.reviews?.length > 0 && <span className="reviews">({selectedProduct.reviews.length} {t("Reviews", "مراجعات")})</span>}
              </p>
            ...
            <div className="dt-buttons">
              ...
              <button onClick={handleBuyNow}>{t("Buy Now", "اشتري الان")}</button>
            </div>
          </section>
        </main>
      </div>)}
    ...
    >
      ...
      <main id='comments-main-container'>
        <div className="pr-comments-container">
          <h3>{t("Customer Reviews", "مراجعات الزبائن")}</h3>
          ...
          )) : <p>{t("No reviews yet.", "لا توجد مراجعات بعد.")}</p>}
          ...
          {user ? (
            <div className="add-comment-box">
              ...
              <div className="input-area">
                <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder={t("Write your comment...", "اكتب تعليقك...")} rows="3" />
                <button className="post-btn" onClick={handleSubmitReview} disabled={reviewLoading}>{t("Post Review", "نشر المراجعة")}</button>
              </div>
            </div>
          ) : <p>
              {t("Please", "يرجى")}{" "}
